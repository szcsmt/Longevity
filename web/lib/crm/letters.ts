...
/* ── The first two letters, written as a person writes ──

   Gmail sorts mail into Primary or Promotions from three things: the sender's
   reputation, the recipient's history with them, and the content. Ours is a
   three-week-old domain with no reputation and no history, so content is the
   only one of the three we can touch — and the designed letter is, correctly,
   read as a promotion: logo image, gold buttons, four links, prices, and the
   List-Unsubscribe header that keeps it out of spam.

   So the two letters whose whole job is to START a conversation drop all of
   it. No image, no button, two links, short. This is not a lesser version of
   the brand letter; it is a different kind of message — a reply to somebody
   who wrote to us two minutes ago, which is what it actually is.

   From day 10 the full design returns. By then the customer has either replied
   (in which case Gmail has learned the sender and the design costs nothing) or
   they have not, and a letter that looks like something is more likely to win
   them back than another plain note.

   Deliberately absent: the essentials table, the price line, the logo, the
   buttons, and — on the welcome — the opt-out. A direct answer to somebody's
   own enquiry is not a mailing, and mailer.ts leaves the unsubscribe headers
   off it for the same reason. */

const PLAIN_INK = '#202124';
const PLAIN_LINK = '#1a5c38';

/** A paragraph in the plain letters. */
const pp = (html: string) => `<p style="margin:0 0 14px">${html}</p>`;

const plainLink = (href: string, label: string) =>
  `<a href="${href}" style="color:${PLAIN_LINK}">${label}</a>`;

function plainShell(body: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<div style="max-width:560px;padding:8px 4px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;font-size:15px;line-height:1.55;color:${PLAIN_INK};">
${body}
</div>
</body>
</html>`;
...
/** The sign-off: a name, a place, a number. No links, no logo, no slogan. */
function plainSignOff(l: Lead): string {
  const me = letterIdentity(l);
  return pp(
    `Best regards,<br>${esc(me.name)}<br>${esc(me.title)}` +
    (me.phone ? `<br>${esc(me.phone)}` : ''),
  );
}

/** Day 0, plain. Answers the form they just filled in, and asks one question. */
export function plainWelcomeEmail(l: Lead): { subject: string; html: string } {
  const name = firstName(l);
  const brochure = docUrl(l, 'overview');
  const isBrochure = l.form_type === 'brochure_request';

  const opening = isBrochure
    ? `Thank you for asking about Longevity Resort. Here is the brochure: ${plainLink(brochure, 'Longevity Resort, 13 pages')}.`
    : l.villa
      ? `Thank you for your enquiry about ${esc(l.villa)}. I am putting the details together for you now — availability, exact pricing and how reservation works.`
      : `Thank you for your enquiry. I am putting the details together for you now.`;

  return {
    subject: isBrochure
      ? 'Your Longevity Resort brochure'
      : `Your enquiry${l.villa ? `, ${esc(l.villa)}` : ''}`,
    html: plainShell(
      pp(`Hi ${esc(name)},`) +
      pp(opening) +
      pp(`We are a managed residence community at Plai Laem on Koh Samui, built around a
          physician-led longevity centre. Owners use their residence part of the year and we
          look after it, and their guests, the rest.`) +
      (isBrochure ? '' : pp(`If it is easier to talk, here is my calendar: ${plainLink(`${SITE}/c?l=${l.id}&t=Book+a+call&d=booking`, 'pick a time that suits you')}.`)) +
      pp(`What would be most useful to know first — the residences, the returns, or the timing?`) +
      plainSignOff(l),
    ),
  };
}

/** Day 3, plain. One nudge, and an explicit invitation to say no. */
export function plainReminderEmail(l: Lead): { subject: string; html: string } {
  return {
    subject: `Following up${l.villa ? ` on ${esc(l.villa)}` : ''}`,
    html: plainShell(
      pp(`Hi ${esc(firstName(l))},`) +
      pp(`Just following up on what I sent a few days ago. A decision like this takes time,
          so there is no rush at all.`) +
      pp(`If a question has come up about the residences, the pricing or the reservation
          process, send it over and I will answer it. And if the timing is simply not right,
          one line saying so is absolutely fine — I will stop writing.`) +
      plainSignOff(l),
    ),
  };
}
