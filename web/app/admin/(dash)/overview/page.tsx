...
export default async function Dashboard() {
  ...
  const trendMax = Math.max(1, ...s.byDay.map((d) => d.count));
  const trendTotal = s.byDay.reduce((n, d) => n + d.count, 0);
  const attentionCount = s.attention.overdue.length + s.attention.untouched.length;
    ...
    <>
      ...
      <div className="crm-grid crm-stats" style={{ marginBottom: 16 }}>
        ...
        <div className="crm-card crm-stat">
          ...
          <div className="v tabnum">
            {s.pipelineValue ? fmtTHBShort(s.pipelineValue) : '—'}
            {s.wonValue ? <small>+{fmtTHBShort(s.wonValue)} won</small> : null}
          </div>
        </div>
      </div>
      ...
      <div className="crm-grid crm-cols-2">
        ...
        <div className="stack">
          ...
          {/* New leads per day, last 30 days */}
          <div className="crm-card">
            <h3>
              Lead trend
              <span style={{ marginLeft: 8, fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: 'var(--c-mut-2)' }}>
                · {trendTotal} in 30 days
              </span>
            </h3>
            {trendTotal === 0 ? (
              <div className="empty">No leads in the last 30 days yet.</div>
            ) : (
              <>
                <div className="trend">
                  {s.byDay.map((d) => (
                    <div
                      className="trend-col"
                      key={d.day}
                      title={`${new Date(d.day).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} — ${d.count} ${d.count === 1 ? 'lead' : 'leads'}`}
                    >
                      <span
                        className={`trend-bar${d.count ? '' : ' zero'}`}
                        style={{ height: `${Math.max(d.count ? 8 : 2, (d.count / trendMax) * 100)}%` }}
                      />
                    </div>
                  ))}
                </div>
                <div className="trend-axis">
                  <span>{new Date(s.byDay[0].day).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                  <span>today</span>
                </div>
              </>
            )}
          ...
          {/* Conversion funnel — how far leads get */}
          <div className="crm-card">
            <h3>
              Conversion funnel
              {(s.byStage.won || 0) + s.lost > 0 && (
                <span style={{ marginLeft: 8, fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: 'var(--c-mut-2)' }}>
                  · {s.wonRate}% win rate on decided
                </span>
              )}
            </h3>
            {s.funnel.map((f) => (
              <div className="bar-row funnel-row" key={f.stage}>
                <span className="lab">{f.label}</span>
                <span className="bar-track"><span className="bar-fill" style={{ width: `${f.pct}%` }} /></span>
                <span className="num tabnum">{f.reached}<small className="pct"> {f.pct}%</small></span>
              </div>
            ))}
            {s.lost > 0 && (
              <div className="crm-meta" style={{ marginTop: 10 }}>
                {s.lost} lost {s.lost === 1 ? 'lead' : 'leads'} excluded from the funnel.
              </div>
            )}
          </div>
        ...
        {/* Right: attention + recent + tasks */}
        <div className="stack">
          {/* What needs a human today */}
          <div className={`crm-card${attentionCount ? ' attention' : ''}`}>
            <h3>Needs attention{attentionCount ? ` · ${attentionCount}` : ''}</h3>
            {attentionCount === 0 && <div className="empty">All clear — nothing overdue, no untouched leads.</div>}
            {s.attention.overdue.map((o) => (
              <Link key={o.taskId} href={`/admin/leads/${o.leadId}`} className="crm-row att-row">
                <div style={{ minWidth: 0 }}>
                  <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{o.title}</div>
                  <div className="crm-meta">{o.leadName} · overdue</div>
                </div>
                <span className="crm-meta" style={{ color: 'var(--c-hot)', whiteSpace: 'nowrap' }}>{fmtDay(o.due)}</span>
              </Link>
            ))}
            {s.attention.untouched.map((u) => (
              <Link key={u.leadId} href={`/admin/leads/${u.leadId}`} className="crm-row att-row">
                <div style={{ minWidth: 0 }}>
                  <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.leadName}</div>
                  <div className="crm-meta">new lead · untouched for {u.ageDays} {u.ageDays === 1 ? 'day' : 'days'}</div>
                </div>
                <span className={`badge ${u.score}`}>{u.score}</span>
              </Link>
            ))}
          </div>
          ...
          <div className="crm-card">
            <h3>Recent leads</h3>
            {recent.length === 0 && <div className="empty">No leads yet — new enquiries land here.</div>}
            {recent.map((l) => (
          ...
          <div className="crm-card">
            ...
            {upcoming.map(({ t, l }) => {
              const over = t.due && t.due.slice(0, 10) < nowIso.slice(0, 10);
              return (
